...
| `.github/workflows/ci.yml` | `name: ci`. Two jobs. `docker`, displayed as **Build Image**: buildx over `linux/amd64,linux/arm/v7,linux/arm64/v8`, GHA build cache, nothing pushed on a pull request. `verify_published`, a matrix displayed as **Verify Published Image (amd64)** and **(arm64)**: `needs: docker`, `master` only, pulls the tag that was just pushed and runs `pytest -m smoke` against it. |
